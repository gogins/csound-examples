...
<CsInstruments>
/*
I am going to TRY to pull out some instrs from this orchestra
by encapsulating the whole orchestra into one opcode, and 
currying the opcode calls for different instr sounds.

There appears to have been a removal of zak channel code.

Some tables are replaced at the same number. This makes the code 
harder to adapt.

I hear what I think is aliasing in some of the modulations.
*/
sr = 48000
ksmps = 100
nchnls = 2
...
k1 poscil3 p6-p5,i1,int(frac(p8)*100+.5),int(p8)
k1 = p5+k1
;for feedback
kk3 init (p9=0? .001 : p9) ;min rvrb
if p12=0 goto j10
kk3 poscil3 p10-p9,p11,int(frac(p12)*100+.5),int(p12)
kk3 = p9+kk3 ;variable rvt
j10: p20 = (p20=0? 1 : -1) ;p20=pos rvtfac if 0, neg if ne 0
k2 = p20*exp(-6.9075*k1/kk3) ;rvt fac = e**lpt/rvt
al2 init 1 ;p21=rate/dur for poscil3 al2
if p22=0 goto j20 ;p22=nf for al2
al2 poscil3 1,p21/p3,p22,0 ;poscil3 tra +&-rvtfac
...
a1 = al1
if p24=0 goto j30
a1 poscil3 a1,p23/p3,int(frac(p24)*100+.5),int(p24) ;zvamp
...
i31 = frac(p17) ;min
i32 = it1-i31 ;dif
k4 poscil3 i32,p18/p3,frac(p19)*100+.5,int(p19) ;wrong phase
kk5 = i31+k4
kk6 = 1-kk5
...
k6 table k6*i4,i3
a1 = a1*k6
j80: a1 poscil3 a5,i9+a1,int(p12),0
ga15 = ga15+a1
outs a1*kk5,a1*kk6
endin
...
f1 0 65537 10 1
f2 0 65537 10 1 -.5 -.25 .1
f3 0 65537 10 1 .05 -.5 .05 .5 .05 -.3 .05 1
f4 0 128 7 0 64 1 64 0
f5 0 65537 10 1 .05 -.6
f6 0 65537 10 1 -.6 .15
f7 0 17 5 1 16 50
f8 0 17 5 1 16 16
...
f3 31 65537 10 1 .15 -.6 .2 -.5 .3 -.4 .3 .3
