; PEACE BE WITH YOU 2 (2015) for realtime Csound6 - by Arthur B. Hunkins
; Includes HTML5 performance GUI for all size Android smartphones and tablets
;   GUI is at bottom of file, and contains 16 sliders and 9 momentary-contact buttons
...
a2      atonex a1, 250 * gkcps, 6
        if p5 > 0 then
        if p5 == 3 then
kpan    rspline -.2, .3, .25, .4
kpan    = (kpan < 0? 0: kpan)
        else
ktrig   trigger kamp, 1, 0
        if ktrig > 0 then
        if p5 == 1 then
kpan    chnget "slider13"
        else
kpan    rnd31 .5, .8
kpan    = kpan + .5
        endif
...
a2      atonex a1, 250 * gkcps2, 6
        if p5 > 0 then
        if p5 == 3 then
kpan    rspline .1, .6, .25, .4
        else
ktrig   trigger kamp, 1, 0
        if ktrig > 0 then
        if p5 == 1 then
kpan    chnget "slider14"
        else
kpan    rnd31 .5, .8
kpan    = kpan + .5
        endif
...
a2      atonex a1, 250 * gkcps3, 6
        if p5 > 0 then
        if p5 == 3 then
kpan    rspline .4, .9, .25, .4
        else
ktrig   trigger kamp, 1, 0
        if ktrig > 0 then
        if p5 == 1 then
kpan    chnget "slider15"
        else
kpan    rnd31 .5, .8
kpan    = kpan + .5
        endif
...
kpan    init 1
kamp    chnget "slider10"
kamp    port kamp * 10000, .05
krand   rspline .6, 1, .3, .8
kfreq	rspline .5, 2.5, 2, 2
gkndx4  chnget "slider12"
gkndx4  port gkndx4 * 8, .05
kndx2   jitter .5, .8, 1
kadd	jspline .15, .7, 1
gkcps4  chnget "slider11"
gkcps4  = (gkcps4 == 0? gkcps4 + 1: gkcps4)
a1	foscil kamp * krand, gkcps4, 400 + (kfreq/gkcps4), p4 + kadd, gkndx4 * (abs(kndx2) + 1), 1
a2      atonex a1, 250 * gkcps4, 6
        if p5 > 0 then
        if p5 == 3 then
kpan    rspline .7, 1.2, .25, .4
kpan    = (kpan > 1? 1: kpan)
        else
ktrig   trigger kamp, 1, 0
        if ktrig > 0 then
        if p5 == 1 then
kpan    chnget "slider16"
        else
kpan    rnd31 .5, .8
kpan    = kpan + .5
        endif
...
function slider11(value) {
	var numberValue = parseFloat(value);
	document.querySelector('#slider11Output').value = numberValue.toFixed(2);
...
function slider13(value) {
	var numberValue = parseFloat(value);
	csound.setControlChannel('slider13', numberValue);
}
function slider14(value) {
	var numberValue = parseFloat(value);
	csound.setControlChannel('slider14', numberValue);
}
function slider15(value) {
	var numberValue = parseFloat(value);
	csound.setControlChannel('slider15', numberValue);
}
function slider16(value) {
	var numberValue = parseFloat(value);
	csound.setControlChannel('slider16', numberValue);
}
...
<b>PEACE BE WITH YOU 2 (2015)</b> &nbsp; Art Hunkins
...
<tr>
<td>
<label for=slider10>10Amplitd4</label>
<td>
<input type=range min=0 max=1 value=0 id=slider10 step=0.001 oninput="slider10(value)">
</tr>
<tr>
<td>
<label for=slider11>11FreqFac4</label>
<td>
<input type=range min=1 max=3.8 value=1 id=slider11 step=0.001 oninput="slider11(value)">
<td>
<output for=slider8 id=slider11Output>1.00</output>
</tr>
<tr>
<td>
<label for=slider12>12 Timbre4</label>
<td>
<input type=range min=0 max=1 value=0 id=slider12 step=0.001 oninput="slider12(value)">
</tr>
<tr>
<td>
<label for=slider13>13 Pan1</label>
<td>
<input type=range min=0 max=1 value=0 id=slider13 step=0.001 oninput="slider13(value)">
</tr>
<tr>
<td>
<label for=slider14>14 Pan2</label>
<td>
<input type=range min=0 max=1 value=0 id=slider14 step=0.001 oninput="slider14(value)">
</tr>
<tr>
<td>
<label for=slider15>15 Pan3</label>
<td>
<input type=range min=0 max=1 value=0 id=slider15 step=0.001 oninput="slider15(value)">
</tr>
<tr>
<td>
<label for=slider16>16 Pan4</label>
<td>
<input type=range min=0 max=1 value=0 id=slider16 step=0.001 oninput="slider16(value)">
</tr>
</table>
...
<td>
<button class="button" onclick="butt1()">ModFac1</button>
<td>
<button class="button" onclick="butt2()">ModFac2</button>
<td>
<button class="button" onclick="butt3()">ModFac3</button>
<td>
<button class="button" onclick="butt4()">ModFac4</button>
<td>
<button class="button" onclick="butt5()">ModFac5</button>
</tr>
<td>
<button class="button" onclick="butt6()">PnMode1</button>
<td>
<button class="button" onclick="butt7()">PnMode2</button>
<td>
<button class="button" onclick="butt8()">PnMode3</button>
<td>
<button class="button" onclick="butt9()">PnMode4</button>
</tr>
</table>
