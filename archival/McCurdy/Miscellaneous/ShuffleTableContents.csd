...
ih		 	FLbox  	"In the second row of GUI items the user can specify a value  ", 	1,      5,     14,    490,    20,     5, 400
ih		 	FLbox  	"which can then be added or multiplied to each value in the   ", 	1,      5,     14,    490,    20,     5, 420
ih		 	FLbox  	"table. Negative values are allowed. The procedure will       ", 	1,      5,     14,    490,    20,     5, 440
ih		 	FLbox  	"protect against out of range values.                         ", 	1,      5,     14,    490,    20,     5, 440
ih		 	FLbox  	"'Add Rand.' will add a random value within the range zero to ", 	1,      5,     14,    490,    20,     5, 460
ih		 	FLbox  	"'Value' to each item in the table. A new random number is    ", 	1,      5,     14,    490,    20,     5, 480
ih		 	FLbox  	"generated for each addition. 'Mult. Rand.' multiplies each   ", 	1,      5,     14,    490,    20,     5, 500
ih		 	FLbox  	"value by a random number.                                    ", 	1,      5,     14,    490,    20,     5, 520
...
instr	1	;SHUFFLE TABLE CONTENTS
	...
	icount 		init 	1			;INITIALISE COUNTER
	LOOPBEGIN:					;LABEL
	ifn1	random	0,iNumItems			;FIRST RANDOM TABLE ITEM INDEX
	ifn2	random	0,iNumItems			;SECOND RANDOM TABLE ITEM INDEX
	ival1	table	ifn1, giTable			;READ FIRST VALUE FROM TABLE LOCATION
	ival2	table	ifn2, giTable			;READ SECOND VALUE FROM TABLE LOCATION
		tableiw	ival2, ifn1, giTable		;ITEM 2 WRITTEN TO ITEM 1'S LOCATION
		tableiw	ival1, ifn2, giTable		;ITEM 1 WRITTEN TO ITEM 2'S LOCATION
	;	OPCODE  COUNTER | INCREMENT | LIMIT | LABEL TO LOOP BACK TO
...
instr	5	;PERFORM RETROGRADE
	...
	ival		table		iNumItems-icount-1, giTable	;READ VALUE FROM TABLE
	...
	ival		table		icount,iTableBuffer
			tableiw		ival,icount,giTable
			loop_lt		icount,1,iNumItems,LOOP2BEGIN
...
instr	7	;ADD VALUE
	...
	iadd	init	i(gkval)			;DERIVE I-TIME VALUE OF 'VALUE' FLTK NUMBER BOX
	iNumItems	=	ftlen(giTable)		;DERIVE THE NUMBER OF ITEMS IN THE FUNCTION TABLE
	icount	init	0                               ;INITIALISE LOOP COUNTER
	LOOPBEGIN:                                      ;LABEL - LOOPING LOOPS BACK TO HERE
	ival	table	icount,giTable                  ;READ VALUE FROM TABLE
	ival	=	ival+iadd                       ;ADD VALUE
	ival	limit	ival,0,127                      ;FIX IF NEW VALUE IS BEYOND MIDI RANGE
		tableiw	ival,icount,giTable             ;WRITE NEW VALUE INTO LOCATION OF ORIGINAL VALUE
		loop_lt	icount,1,iNumItems,LOOPBEGIN    ;LOOPING CONSTRUCTION
...
instr	8	;MULTIPLY VALUE
	...
	imult	init	i(gkval)			;DERIVE I-TIME VALUE OF 'VALUE' FLTK NUMBER BOX
	iNumItems	=	ftlen(giTable)		;DERIVE THE NUMBER OF ITEMS IN THE FUNCTION TABLE
	icount	init	0				;INITIALISE LOOP COUNTER
	LOOPBEGIN:					;LABEL - LOOPING LOOPS BACK TO HERE
	ival	table	icount,giTable			;READ VALUE FROM TABLE
	ival	=	ival*imult			;MULTIPLY BY VALUE
	ival	limit	ival,0,127			;FIX IF NEW VALUE IS BEYOND MIDI RANGE
		tableiw	ival,icount,giTable		;WRITE NEW VALUE INTO LOCATION OF ORIGINAL VALUE
		loop_lt	icount,1,iNumItems,LOOPBEGIN	;LOOPING CONSTRUCTION
	event_i	"i", 2, 0, 0				;CALL INSTRUMENT THAT PRINTS TABLE VALUES
...
instr	9	;ADD RANDOM VALUE
	...
	LOOPBEGIN:                                      ;LABEL - LOOPING LOOPS BACK TO HERE
	ival	table	icount,giTable                  ;READ VALUE FROM TABLE
	iRAdd	random	0,iadd				;CREATE A RANDOM VALUE
...
instr	10	;MULTIPLY BY RANDOM VALUE
	...
	LOOPBEGIN:					;LABEL - LOOPING LOOPS BACK TO HERE
	ival	table	icount,giTable			;READ VALUE FROM TABLE
	iRMult	random	0,imult				;CREATE A RANDOM VALUE
...
instr	11	;SORT INTO ASCENDING ORDER
	...
	iNumItems	=	ftlen(giTable)			;DERIVE THE NUMBER OF ITEMS IN THE FUNCTION TABLE
	imax	table	0,giTable				;STARTING VALUE IN THE SEARCH FOR  FOR THE MAXIMUM
	icount0	init	1					;COUNTER STARTS AT ONE (WE'VE ALREADY READ VALUE ZERO)
	LOOP0BEGIN:						;LABEL - BEGINING OF SUMMING LOOP
	  ival	table	icount0,giTable				;READ VALUE FROM TABLE
	  imax = (ival>=imax?ival:imax)				;IF VAL IS BIGGER THAN CURRENT MAXIMUM THEN MAKE IT THE NEW MAXIMUM
	loop_lt	icount0,1,iNumItems,LOOP0BEGIN			;LOOP BACK TO 'LOOP0BEGIN' UNTIL LOOPING COMPLETE
	...
	icount1	init	0					;INITIALISE MAIN LOOP COUNTER
	LOOP1BEGIN:						;LABEL - MAIN LOOPING LOOPS BACK TO HERE
	  ...
	  LOOP2BEGIN:						;LABEL - SUB-LOOP LOOPS BACK TO HERE
	    ival	table	icount2,giTable			;READ VALUE FROM TABLE...
	    ...
	    loop_lt	icount2,1,iNumItems,LOOP2BEGIN		;SUB-LOOP LOOPS BACK TO 'LOOP2BEGIN' UNTIL COUNTER REACH THE END OF THE TABLE
	  tableiw	imin,icount1,iTableBuffer		;WRITE THE MINIMUM VALUE FOUND INTO THE BUFFER TABLE
	  tableiw	imax,iloc,giTable			;SET THE MINIMUM VALUE IN THE ORIGINAL TABLE TO THE MAXIMUM VALUE SO THAT WE DON'T FIND IT AGAIN
	  loop_lt	icount1,1,iNumItems,LOOP1BEGIN		;LOOPING CONSTRUCTION
	;COPY BUFFER TABLE BACK INTO ORIGINAL TABLE
	icount		=		0
	LOOP3BEGIN:
	ival		table		icount,iTableBuffer
			tableiw		ival,icount,giTable
			loop_lt		icount,1,iNumItems,LOOP3BEGIN
			event_i		"i", 2, 0, 0			;PRINT TABLE VALUES
...
<CsScore>
i "tables" 0 3600
i 2 0.01 0	;PRINT TABLE ITEMS TO CONSOLE AND FL BOXES AT THE BEGINNING OF PERFORMANCE
