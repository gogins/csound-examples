...
;
; LayerInstruments  klayers,iInstr,idur,krelease
; 
; Initialisation
; --------------
...
; krelease --  if non-zero, release stages in envelopes in the called instruemnt are allowed to complete - otherwise are deactivated immediately (possibly resulting in clicks)

opcode	LayerInstruments,0,KiiK
	klayers,iInstr,idur,krelease	xin			;READ IN INPUT ARGUMENTS
	...
	if ktrig=1 then					;IF A TRIGGER IS RECEIVED (CHANGE IN klayers)...
	 turnoff2	iInstr,0,krelease		;TURN OFF ALL INSTANCES OF THE CALLED INSTRUMENT
	...
	ktrigdelay	delayk	ktrig,ksmps/kr		;DELAYED VERSION OF THE 'TURN OFF LAYERS' TRIGGER. INSTRUMENT RESTART MUST HAPPEN AFTER A K PERIOD.
