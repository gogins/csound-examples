...
;			LABEL     | WIDTH | HEIGHT | X | Y
		FLpanel	"gbuzz",     500,    660,    0,  0

;				FLBUTBANK 	TYPE | NUMX | NUMY | WIDTH | HEIGHT | X | Y | IOPCODE | P1 | P2 | P3
gkFLTK_MIDI, ihFLTK_MIDI	FLbutBank	4,      1,     2,     20,      40,    5,  5,    -1	;0,     1,    0,  -1
...
gkharm2, gklh2, ihharm2, ihlh2	FLjoy	"X - Num.Harms. Y - Lowest Harm.",	1,      200,  1,      200,   -1,    -1,   idharm2,  idlh2,    490,    180,     5, 200
...
FLsetColor	255,255,200,ih
ih		 	FLbox  	"", 					7,      1,     14,    495,     70,     5,  585		;box
...
;SLIDERS				     			MIN | MAX | EXP | TYPE | DISP | WIDTH | HEIGHT | X | Y
gkDec, ihDec		FLslider	"Decay",  		0.01,   15,  -1,   23,   -1,     230,    20,     10, 615
gkSus, ihSus		FLslider	"Sustain",  		0,       1,   0,   23,   -1,     230,    20,    260, 615
;SET INITIAL VALUES		VALUE | HANDLE 
		FLsetVal_i	1, 	ihDec
		FLsetVal_i	1, 	ihSus
...
instr	1		;GBUZZ INSTRUMENT
	...
	asig	gbuzz 	gkamp,        kfreq,        gkharm,            gklh,       kmul,    gicos+i(gkwave)
	rireturn

	aenv	expsegr	0.01,0.01,1,i(gkDec),i(gkSus)+0.01,0.01,0.01		;AMPLITUDE ENVELOPE
	outs	asig * aenv, asig * (aenv-0.01)				;SEND AUDIO OUTPUT TO THE SPEAKERS
endin

instr	2; UPDATE FLTK TEXT BOXES
	ktrig0 changed	gkharm2,gklh2
	if ktrig0==1 then
	 if gkdisplay==0 then
	  gklh	=	int(gklh2)
	  gkharm	=	int(gkharm2)
...
instr	3	;REDIFINE THE USER WAVEFORM WHENEVER A CHANGE IS MADE
