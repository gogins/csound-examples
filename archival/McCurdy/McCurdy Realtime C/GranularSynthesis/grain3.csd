...
gkpch, ihpch		FLslider	"Pitch",					0,                1,   0,    23,     -1,        490,     25,    5,   150
gkfmd, ihfmd		FLslider	"Pitch Offset",					0,              0.3,   0,    23,     idfmd,     490,     25,    5,   200
...
gksfn, ihsfn		FLcount  "Sound File Input", 	1,      5,    1,     100,     2,    150,     25,    50,805,   -1 
...
ih		 FLbox  "Sound Files: 1 - Voice . 2 - Drum Loop . 3 - Classical Guitar . 4 - Sine . 5 - Buzz", 																								1,      1,      12,    250,     30,    0, 845
...
opcode	Grain3b,a,kkkkkkiiikkiiik
	...
	if iPhsMode==1 then				; IF SPEED CONTROL PHASE MODULATION MODE IS SELECTED...
	 kpch		=	kpch - kphs		; COMPENSATE PITCH TRANSPOSITION VALUE TO PREVENT UNWANTED TRANSPOSITION
	 kphs		phasor	kphs/(nsamp(isfn)/sr)	; CREATE A MOVING PHASE VALUE WITH FREQUENCY APPROPRIATE TO THE LEBGTH OF THE SOURCE FUNCTION TABLE AND THE SAMPLE RATE
	endif

	if gksfn<=3 then				; IF INPUT TABLE FOR GRAINS IS A SOUND FILE...
	  kpch 		= 	(sr*kpch)/ftlen(isfn)	; SCALE PITCH VALUE
	else						; OTHERWISE... (I.E. A SINGLE CYCLE WAVEFORM HAS BEEN SELECTED)
	  kfmd		=	kfmd * kpch		; SCALE PITCH MODULATION 
	endif
	...
	asig	grain3	kpch, kphs, kfmd, kpmd, kdur, kdens, imaxovr, isfn, iwfn, kfrpow, kprpow , iseed, imode
	xout	asig
...
instr	1
	if gksfn<4 then					;IF AN AUDIO FILE IS SELECTED...
	 kpch	scale	gkpch,2,-2
	else						;OTHERWISE A WAVEFORM HAS BEEN SELECTED 
	 kpch	logcurve	gkpch,4
	 kpch	scale		gkpch,2000,20
	 
	endif
	
	FLprintk2	kpch,gidpch
	...
	if iMIDIflag=1&&gkMIDIToPch=1 then		;IF THIS IS A MIDI ACTIVATED NOTE AND MIDI-TO-PITCH SWITCH IS ON... 
		...
		kpch		portk	kpch, kporttime	;USE THE FLTK SLIDER VALUE
	endif					;END OF THIS CONDITIONAL BRANCH
