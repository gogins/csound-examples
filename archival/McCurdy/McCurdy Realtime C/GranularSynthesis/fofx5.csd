...
<CsOptions>
;USE THE FOLLOWING LINE OF CODE IS A MIDI KEYBOARD *IS* PRESENT
;-odac -M0 -dm0
;USE THE FOLLOWING LINE IF NO MIDI KEYBOARD IS PRESENT
...
#define	FORMANT(N)
	#
	invals	table	0, giBF1+(i(gkvoice)*15)+$N-1						;NUMBER OF DATA ELEMENTS IN EACH TABLE
	invals	=	invals-1								;
	k$Nform 		tablei		1+(kndx*invals), giBF1+(i(gkvoice)*15)+$N-1	;READ FORMANT FREQUENCY FROM TABLE
	k$Ndb 			tablei		1+(kndx*invals), giBDb1+(i(gkvoice)*15)+$N-1	;READ DECIBEL VALUE FROM TABLE
	k$Namp			=		ampdb(k$Ndb)					;CONVERT TO AN AMPLITUDE VALUE                                                
	k$Nband 		tablei		1+(kndx*invals), giBBW1+(i(gkvoice)*15)+$N-1	;READ BANDWIDTH FROM TABLE
	#
