...
#define	FORMANT(N)
	...
	invals	=	invals-1								;
	k$N.form 		tablei		1+(kndx*invals), giBF1+(i(gkvoice)*15)+$N-1	;READ FORMANT FREQUENCY FROM TABLE
	k$N.db 			tablei		1+(kndx*invals), giBDb1+(i(gkvoice)*15)+$N-1	;READ DECIBEL VALUE FROM TABLE
	k$N.amp			=		ampdb(k$N.db)					;CONVERT TO AN AMPLITUDE VALUE                                                
	k$N.band 		tablei		1+(kndx*invals), giBBW1+(i(gkvoice)*15)+$N-1	;READ BANDWIDTH FROM TABLE
	#
